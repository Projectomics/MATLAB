function fileName = shuffle_both_constraints_laterality(inputFile, region, filterBinMatrix)
    %clear all;
        ...
        while count <= 100000
            if isFullShuffle
                ...
                if((topLeft == bottomRight) & (bottomLeft == topRight) & (topLeft ~= bottomLeft) & (topRight ~= bottomRight))
                    first = filterBinMatrix.parcels{randCol1};
                    second = filterBinMatrix.parcels{randCol2};
