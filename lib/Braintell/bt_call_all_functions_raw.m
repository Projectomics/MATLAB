function p = bt_call_all_functions_raw(neuronArray, matFile)
    addpath('./lib/');
    ...
    if(length(neuronArray) <= 2) % 2 neurons or less
        ...
        %        end
        %end
        %rawBinMatrix = open_raw_data_file('Presubiculum_raw_morphologyMatrix_20201005092953.csv');
        rawBinMatrix = open_data_file(matFile);
